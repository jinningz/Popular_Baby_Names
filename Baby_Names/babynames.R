...
## See the LICENSE file in the root directory of 
## http://github.com/jinningz/DataAnalysis
...
for (i in 1880:2013) {
        ...
        datafile <- read.table(paste("./names/yob", i, ".txt", sep=""), 
                               sep=",", stringsAsFactors = FALSE, 
                               col.names=c("name", "sex", "number"))
        datafile <- cbind(year=i, datafile)
        rank <- c(1:nrow(datafile[datafile$sex == "F",]), 
                  1:nrow(datafile[datafile$sex == "M",]))
...
## Ranks over time
qplot(year, rank, data = singleNameData [singleNameData $sex == "M",], 
      colour = "red") + geom_line() + scale_y_reverse()
qplot(year, rank, data = singleNameData [singleNameData $sex == "M",], 
      colour = "red") + geom_line() 
        + geom_smooth(method = "loess", size = 1) + scale_y_reverse()

## Number of occurances over time
qplot(year, number, data = singleNameData [singleNameData $sex == "M",], 
      colour = "red") + geom_line()
qplot(year, number, data = singleNameData [singleNameData $sex == "M",], 
      colour = "red") + geom_line() 
        + geom_smooth(method = "loess", size = 1)

## The plots should be exactly the same as shown in the 
## following websites. Filter for US data.
...
qplot(year, rank, data = singleNameData [singleNameData $sex == "M",], 
      colour = "red") + geom_line() + scale_y_reverse()
qplot(year, rank, data = singleNameData [singleNameData $sex == "F",], 
      colour = "red") + geom_line() + scale_y_reverse()
...
qplot(year, rankDiff_yy, data = mergedData[mergedData$sex == "M" 
                                & mergedData$name %in% topNames, ], 
      colour = name) + geom_line()

## Top 30 Names in 2013 (both gender)
topNames2013 <- mergedData[mergedData$rank <= 30 
                           & mergedData$year == 2013, "name"]
length(topNames2013)
## Changes since 2000
qplot(year, rankDiff_yy, 
      data = mergedData[mergedData$year >= 2000 
                        & mergedData$sex == "M" 
                        & mergedData$name %in% topNames2013, ], 
      colour = name) + geom_line() + scale_y_reverse()

## Historical Development of the Top 5 boy Names in 2013
top5MNames2013 <- mergedData[mergedData$rank <= 5 
                             & mergedData$year == 2013 
                             & mergedData$sex == "M", "name"]
length(top5MNames2013)
qplot(year, rankDiff_yy, 
      data = mergedData[mergedData$year >= 1970 
                        & mergedData$sex == "M" 
                        & mergedData$name %in% top5MNames2013, ], 
      colour = name) + geom_line() + scale_y_reverse()
qplot(year, rankDiff_yy, 
      data = mergedData[mergedData$year >= 2000 
                        & mergedData$sex == "M" 
...
mergedData[mergedData$name == "Liam" 
           & mergedData$sex == "M" 
           & mergedData$year >= 2000,]
...
## Which baby name climb up the ranking chart the most in 2013
## , adjusted for actual ranks
mergedData1 <- mergedData[mergedData$year == 2013 
                          & mergedData$sex == "M" 
                          & mergedData$rank <= 200, ]
...
qplot(year, rank, data = mergedData[mergedData$sex == "M" 
                                    & mergedData$year >= 2000 
                                    & mergedData$name %in% top5MNames2013, ], 
      colour = name) 
